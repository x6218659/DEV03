...
{
	[ObjectSystem]
	public class MyUiLobbyComponentSystem : AwakeSystem<MyUILobbyComponent>
	{
		public override void Awake(MyUILobbyComponent self)
		{
			self.Awake();
		}
	}
	
	public class MyUILobbyComponent : Component
	{
		...
		private async void EnterMap()
		{
			try
			{
				...
				Game.Scene.GetComponent<UIComponent>().Remove(UIType.MyUILobby);
			}
			catch (Exception e)
			{
				Log.Error(e);
